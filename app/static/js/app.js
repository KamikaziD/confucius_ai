...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", executeAgents);
...
function connectWebSocket() {
  ...
  function heartbeat() {
    ...
    // Use a timeout that's slightly longer than the server's ping interval + pong timeout
    pingTimeout = setTimeout(() => {
      console.warn("WebSocket heartbeat failed. Disconnecting.");
      websocket.close();
    }, 45000); // Server ping_interval (30s) + pong_timeout (10s) + buffer
  }
...
async function executeAgents() {
  ...
  try {
    ...
    addLog(
      "System",
      `Task ${data.task_id} accepted. Waiting for results...`,
      false
    );
      ...
      .map(
        (session) => `
            <div class="history-item ${session.status}">
                ...
                <div class="history-query">${session.input.substring(0, 100)}${
          session.input.length > 100 ? "..." : ""
        }</div>
                <div class="history-meta">
                    Duration: ${session.duration?.toFixed(2)}s | Model: ${
...
async function loadPrompts() {
  try {
    ...
    promptsList.innerHTML = Object.entries(prompts)
      .map(
        ([key, prompt]) => `
...
function updateSettingsUI(collections) {
  ...
  // Model settings
  const modelHtml = ["master", "ocr", "info", "rag", "embedding"]
    .map(
      (agent) => `
            ...
            <select id="model-${agent}" onchange="updateModel('${agent}', this.value)">
                ${availableModels
                  .map(
                    (m) => `
                    <option value="${m.name}" ${
                      currentModels[agent] === m.name ? "selected" : ""
                    }>
                        ${m.name}
                    </option>
                `
...
function displayExecutionPlan(plan) {
  ...
  if (!plan) {
    executionPlanDiv.innerHTML =
      '<p class="empty-state">Execution plan not available.</p>';
    return;
  }
  const planHtml = `
...
function clearResults() {
  document.getElementById("executionPlan").innerHTML =
